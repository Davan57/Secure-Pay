...
const FaceAuth = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (response.status === 200 && response.data.face_identity !== "Unknown Person") {
        setMessage(`✅ Success: Recognized as - ${response.data.face_identity}`);
        setIsAuthenticated(true);
        setAuthFailed(false);
      } else {
        setMessage("❌ Authentication Failed: Face not recognized.");
        setIsAuthenticated(false);
        setAuthFailed(true);
      }
    } catch (error) {
      setMessage("❌ Error occurred while recognizing the face.");
      setIsAuthenticated(false);
      setAuthFailed(true);
    }
  };
  ...
  return (
    <div className="h-screen flex flex-col items-center justify-center bg-gradient-to-r from-blue-500 to-purple-600 text-white h-screen flex flex-col items-center justify-center bg-gray-900 text-white flex flex-col items-center justify-center h-screen bg-gray-900 text-white">
      <h2 className="text-2xl mb-4 animate-bounce">Upload Face for Authentication</h2>
